...
PRICE_FOR_REGISTERED_PROPERTY = "1,409,040.00"  # Price for registered property


class TestEmiratiAbuDhabiVarified:
...
Checks movers companies."""
    ...
    def test_TS003_Abu_Dhabi_varified(self):
        ...
        with allure.step("Check that registration message isn't displayed"):
            deed_transfer_property_registration_page = DeedTransferPropertyRegistrationPage()
            assert_that(not_(deed_transfer_property_registration_page.is_in_progress_message_present()),
                        'Message is present')
