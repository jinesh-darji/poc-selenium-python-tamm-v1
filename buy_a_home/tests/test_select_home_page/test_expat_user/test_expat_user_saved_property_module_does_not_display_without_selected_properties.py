...
class TestAddDataToSavedProperties:
    """Select home: Expat user: "Saved properties" button isn't displayed if user didn't select heart for property:
    BUAH-1806"""

    @pytest.mark.smoke
    @pytest.mark.expat_user
    def test_expat_user_saved_property_module_does_not_display_without_selected_properties(self):
        ...
        with allure.step("Select star for the first home"):
            ...
            search_data = search_home_page.get_data_from_home_with_star(
                search_home_page.get_random_data_about_home_from_locator(
                    search_home_page.lst_of_results_price_with_enabled_stars, True, search_home_page))
            assert_that(search_home_page.is_selected_star_present(search_data[0]), "Star wasn't selected for home")

        with allure.step("Select Tamm portal and select Select Home"):
